...
USER_DOMAIN_FROM_DB=`mysql -u"$DBUSER" -p"$DBPASS" -s -N -e "SELECT USERNAME, DOMAIN, DEL_LOCAL, ACCESS_POP3, ACCESS_RECEIVE, AUTO_ENABLED, MAILBOX_DIRECTORY, HOME_WEB  FROM "$DBNAME"."$TABLEIN" WHERE DOMAIN = CONVERT( _utf8 '$u'USING latin1 ) COLLATE latin1_swedish_ci" | sed 's/\*/STAR/g'`
